...
const staggerReveal = (targets: Element[] | NodeListOf<Element>, options = {}) => {
  gsap.from(targets, {
    y: 28,
    opacity: 0,
    duration: 1,
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (tiltTarget) {
    const resetTilt = () => gsap.to(tiltTarget, { rotationX: 0, rotationY: 0, transformPerspective: 1000, duration: 0.8, ease: 'power2.out' });
